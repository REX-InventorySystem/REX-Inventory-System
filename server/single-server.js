...
app.post('/generate-reference-report-pdf', (req, res) => {
  try {
    ...
    let currentY = drawCompanyHeader(doc);
    ...
    doc.fillColor('#1e293b')
       ...
       .fillColor('#64748b')
       .text(` ${referenceData.createdBy || 'Inventory System'}`);
    ...
    doc.fillColor('#64748b')
       ...
       .text(`Generated by ${COMPANY_INFO.name} | ${new Date().toLocaleString()} | Inventory Management System v2.0`, { align: 'center' });
...
app.post('/generate-purchase-pdf', (req, res) => {
  try {
    ...
    doc.fillColor('#1e293b')
       ...
       .fillColor('#64748b')
       .text(` ${purchaseData.createdBy || 'Inventory System'}`);
    ...
    doc.fillColor('#64748b')
       .fontSize(8)
       .text(`${COMPANY_INFO.name} - Purchase Order Document`, { align: 'center' })
...
app.post('/generate-sales-pdf', (req, res) => {
  try {
    ...
    doc.fillColor('#64748b')
       ...
       .text(`${COMPANY_INFO.name} - Sales Invoice | ${COMPANY_INFO.phone} | ${COMPANY_INFO.email}`, { align: 'center' })
       .text(`Generated on: ${new Date().toLocaleString()}`, { align: 'center' });
...
app.post('/generate-inventory-report-pdf', (req, res) => {
  try {
    ...
    doc.fillColor('#1e293b')
       ...
       .fillColor('#64748b')
       .text(` ${reportData.createdBy || 'Inventory System'}`);
    ...
    doc.fillColor('#64748b')
       .fontSize(8)
       .text(`Confidential Inventory Report - ${COMPANY_INFO.name} - For Internal Use Only`, { align: 'center' })
       .text(`${COMPANY_INFO.address} | ${COMPANY_INFO.phone} | ${COMPANY_INFO.email}`, { align: 'center' })
...
function getCSS() {
  return `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .empty-state, .loading, .error-state, .no-data {
      text-align: center;
      padding: 40px;
      color: #94a3b8;
    }
