      ...
      <div class="container">
        ...
        <div class="info-grid">
          ...
          <div>
            ...
            <div class="info-value">${docData.date || new Date(docData.createdAt).toLocaleDateString('en-GB', { timeZone: 'Asia/Kuala_Lumpur' })}</div>
          </div>
          ${entityLabel ? `
          <div>
...
app.get('/api/inventory', async (req, res) => {
  try {
    ...
    const items = await db.collection('inventory').find(query).sort({ createdAt: -1 }).toArray();
    res.json(items);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/inventory/add', async (req, res) => {
  try {
    const item = {
      ...req.body,
      dateAdded: new Date().toLocaleDateString('en-GB', { timeZone: 'Asia/Kuala_Lumpur' }),
      createdAt: new Date()
    };
...
app.post('/api/reference-reports/add', async (req, res) => {
  try {
    ...
    console.error('Reference report error:', error);
    res.status(500).json({ error: error.message });
  }
});
...
app.delete('/api/user', async (req, res) => {
  try {
    ...
    await db.collection('sales').deleteMany({ createdBy: username });
    await db.collection('login_history').deleteMany({ username });
    
    res.json({ success: true, message: 'Account deleted successfully. Inventory data preserved.' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
function addFooter(doc, pageBottom = 750) {
  ...
  doc.fillColor('#64748b')
     .fontSize(8)
     .font('Helvetica')
     .text(COMPANY_INFO.name, 50, footerY, { 
       width: 500,
       align: 'center'
     })
     .text(`${COMPANY_INFO.address} | ${COMPANY_INFO.phone} | ${COMPANY_INFO.email}`, 50, footerY + 10, {
       width: 500,
       align: 'center'
     })
...
app.post('/generate-purchase-pdf', async (req, res) => {
  try {
    ...
    const leftColumn = 50, rightColumn = 300;
    const displayDate = purchaseData.date || new Date().toLocaleDateString('en-GB', { timeZone: 'Asia/Kuala_Lumpur' });
...
app.post('/generate-sales-pdf', async (req, res) => {
  try {
    ...
    const leftColumn = 50, rightColumn = 300;
    const displayDate = salesData.date || new Date().toLocaleDateString('en-GB', { timeZone: 'Asia/Kuala_Lumpur' });
...
app.post('/generate-inventory-report-pdf', async (req, res) => {
  try {
    ...
    const displayDate = reportData.date || new Date().toLocaleDateString('en-GB', { timeZone: 'Asia/Kuala_Lumpur' });
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Login | ${COMPANY_INFO.name} Inventory System</title>
  <style>${getCSS()}</style>
</head>
<body>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Register | ${COMPANY_INFO.name} Inventory System</title>
  <style>${getCSS()}</style>
</head>
<body>
  <div class="auth-container">
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Dashboard | ${COMPANY_INFO.name} Inventory System</title>
  <style>${getCSS()}</style>
</head>
<body>
  <div class="container">
    <div class="topbar">
      <h2>🏢 ${COMPANY_INFO.name} - Inventory Dashboard</h2>
  ...
  <footer>© 2025 ${COMPANY_INFO.name} Inventory Management System</footer>
  ...
  <script>
        ...
        inventoryItems.forEach((item, i) => {
          ...
          body.innerHTML += \`
            <tr>
              ...
              <td class="date-text">\${item.dateAdded || new Date(item.createdAt).toLocaleDateString('en-GB')}</td>
    ...
    async function downloadInventoryReport() {
      try {
        ...
        const dateRange = dateFrom && dateTo ? \`\${dateFrom.split('-').reverse().join('/')} to \${dateTo.split('-').reverse().join('/')}\` : 'All Items';
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Generate Reference Report | ${COMPANY_INFO.name} Inventory System</title>
  <style>${getCSS()}</style>
</head>
<body>
  <div class="container">
    <div class="topbar">
      <h2>📋 Generate Reference Report - ${COMPANY_INFO.name}</h2>
      <div class="topbar-actions">
  ...
  <script>
    ...
    async function loadAvailableItems() {
      try {
        ...
        container.innerHTML = availableItems.length ? '' : '<p class="no-data">No items available</p>';
        ...
        document.getElementById('availableItems').innerHTML = '<p class="error-state">Failed to load items</p>';
      }
    }
    ...
    async function downloadReferencePDF() {
      ...
      try {
        ...
        const saveResponse = await fetch('/api/reference-reports/add', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ referenceData, createdBy: user.username })
        });

        if (!saveResponse.ok) {
          const errorData = await saveResponse.json();
          throw new Error(errorData.error || 'Failed to save reference report');
        }
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Purchase | ${COMPANY_INFO.name} Inventory System</title>
  <style>${getCSS()}</style>
</head>
<body>
  <div class="container">
    <div class="topbar">
      <h2>📥 Purchase (Stock In) - ${COMPANY_INFO.name}</h2>
      <div class="topbar-actions">
        <span>Welcome, <strong id="username"></strong></span>
        <button class="btn small ghost" onclick="toggleTheme()">🌓</button>
        <button class="btn small" onclick="window.location.href='/?page=dashboard'">← Dashboard</button>
    ...
    <div class="card">
      ...
      <div class="form-row">
        ...
        <label>Purchase Date <input type="date" id="purchaseDate" value="${new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kuala_Lumpur' })}"></label>
      </div>
    </div>
  ...
  <script>
    ...
    function updatePurchaseDisplay() {
      ...
      container.innerHTML = selectedPurchaseItems.length ? '' : '<p class="no-data">No items in purchase</p>';
    ...
    async function processPurchase() {
      ...
      try {
        ...
        const purchaseData = {
          date: pDate ? pDate.split('-').reverse().join('/') : new Date().toLocaleString('en-GB'),
        ...
        const response = await fetch('/api/purchases', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ purchaseData, createdBy: user.username })
        });
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Sales | ${COMPANY_INFO.name} Inventory System</title>
  <style>${getCSS()}</style>
</head>
<body>
  <div class="container">
    <div class="topbar">
      <h2>📤 Sales (Stock Out) - ${COMPANY_INFO.name}</h2>
      <div class="topbar-actions">
        <span>Welcome, <strong id="username"></strong></span>
        <button class="btn small ghost" onclick="toggleTheme()">🌓</button>
        <button class="btn small" onclick="window.location.href='/?page=dashboard'">← Dashboard</button>
    ...
    <div class="card">
      ...
      <div class="form-row">
        ...
        <label>Sales Date <input type="date" id="salesDate" value="${new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kuala_Lumpur' })}"></label>
      </div>
    </div>
  ...
  <script>
    ...
    function updateSalesDisplay() {
      ...
      container.innerHTML = selectedSalesItems.length ? '' : '<p class="no-data">No items in sale</p>';
    ...
    async function processSale() {
      ...
      try {
        const user = JSON.parse(localStorage.getItem('currentUser') || '{}');
        const sDate = document.getElementById('salesDate').value;
        const salesData = {
          date: sDate ? sDate.split('-').reverse().join('/') : new Date().toLocaleString('en-GB'),
        ...
        const response = await fetch('/api/sales', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ salesData, createdBy: user.username })
        });
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Statement | ${COMPANY_INFO.name} Inventory System</title>
  <style>${getCSS()}</style>
</head>
<body>
  <div class="container">
    <div class="topbar">
      <h2>📑 Statements & Reports - ${COMPANY_INFO.name}</h2>
      <div class="topbar-actions">
        <span>Welcome, <strong id="username"></strong></span>
        <button class="btn small ghost" onclick="toggleTheme()">🌓</button>
        <button class="btn small" onclick="window.location.href='/?page=dashboard'">← Dashboard</button>
  ...
  <script>
    async function loadReports() {
      try {
        ...
        if (statements.length === 0) {
          container.innerHTML = '<p class="no-data">No reports generated yet.</p>';
        } else {
          statements.sort((a, b) => new Date(b.createdAt || b.date) - new Date(a.createdAt || a.date));
          container.innerHTML = '';
            ...
            reportDiv.innerHTML = \`
              <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                  ...
                  <small>Generated: \${report.date || new Date(report.createdAt).toLocaleString('en-GB', { timeZone: 'Asia/Kuala_Lumpur' })}</small><br>
        ...
        document.getElementById('reportsList').innerHTML = '<p class="error-state">Failed to load reports</p>';
      }
    }
    ...
    async function loadReferenceReports() {
      try {
        ...
        if (!response.ok) throw new Error('Failed to load reference reports');
        ...
        if (referenceReports.length === 0) {
          container.innerHTML = '<p class="no-data">No reference reports generated yet.</p>';
        } else {
        ...
        document.getElementById('referenceReportsList').innerHTML = '<p class="error-state">Failed to load reference reports</p>';
      }
    }
    ...
    async function loadPurchases() {
      try {
        ...
        if (purchases.length === 0) {
          container.innerHTML = '<p class="no-data">No purchase history.</p>';
        } else {
            ...
            purchaseDiv.innerHTML = \`
              <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                  ...
                  <small>Date: \${purchase.date || new Date(purchase.createdAt).toLocaleString('en-GB', { timeZone: 'Asia/Kuala_Lumpur' })} | Supplier: \${purchase.supplier || 'N/A'}</small><br>
        ...
        document.getElementById('purchasesList').innerHTML = '<p class="error-state">Failed to load purchases</p>';
      }
    }
    ...
    async function loadSales() {
      try {
        ...
        if (sales.length === 0) {
          container.innerHTML = '<p class="no-data">No sales history.</p>';
        } else {
            ...
            saleDiv.innerHTML = \`
              <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                  ...
                  <small>Date: \${sale.date || new Date(sale.createdAt).toLocaleString('en-GB', { timeZone: 'Asia/Kuala_Lumpur' })} | Customer: \${sale.customer || 'N/A'}</small><br>
        ...
        document.getElementById('salesList').innerHTML = '<p class="error-state">Failed to load sales</p>';
      }
    }
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Account Settings | ${COMPANY_INFO.name} Inventory System</title>
  <style>${getCSS()}</style>
</head>
<body>
  <div class="container">
    <div class="topbar">
      <h2>⚙️ Account Settings - ${COMPANY_INFO.name}</h2>
      <div class="topbar-actions">
        <span>Welcome, <strong id="username"></strong></span>
        <button class="btn small ghost" onclick="toggleTheme()">🌓</button>
        <button class="btn small" onclick="window.location.href='/?page=dashboard'">← Dashboard</button>
...
function getCSS() {
  return `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .username.current-user { color: var(--success); font-weight: bold; }
    .login-time { font-size: 0.8em; color: #94a3b8; }
    .login-status { padding: 4px 8px; border-radius: 20px; font-size: 0.8em; font-weight: 500; }
    .login-status.success { background: var(--success); color: white; }
    .login-details { display: flex; flex-direction: column; gap: 5px; }
    .detail-item { display: flex; justify-content: space-between; font-size: 0.9em; }
    .detail-label { color: #94a3b8; }
    .detail-value { color: #f1f5f9; font-weight: 500; }
    body.light .detail-value { color: #1e293b; }
